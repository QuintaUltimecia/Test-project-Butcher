...
public class UILineRenderer : Graphic
{
    ...
    public IEnumerable<Vector2> Point { get { return _points; } }

    public float GetDistanceLast(Vector2 target)
    {
        if (_points.Count == 0)
            return float.MaxValue;

        Vector2 first = _points[_points.Count-1];
        Vector2 second = target;
